...
0N_*.ipynb                # the runnable entry points, at the repo root, numbered in pipeline order
  01_download_prices.ipynb  02_import_csv_to_db.ipynb
  03_view_ig_prices.ipynb      04_backtest_strategy.ipynb
  05_download_yahoo_finance.ipynb
...
access to IG. `05_download_yahoo_finance.ipynb` needs network access to Yahoo
Finance but no `.env` and no credentials — its `TICKER`/`PERIOD`/`INTERVAL`
are set directly in the notebook. `03_view_ig_prices.ipynb` only needs
`data/ig_market_data.db` to exist. `02_import_csv_to_db.ipynb` loads one
...
`data/ig_market_data.db` is the hand-off point: `02_import_csv_to_db.ipynb`
writes it (from a CSV `01_download_prices.ipynb` produced),
...
part of the key — it's fixed per table). `02_import_csv_to_db.ipynb` creates
tables via `candle_db.init_candles_table()` — keep it as the single source of
...
**`01_download_prices.ipynb` (producer).** Downloads candles from the IG REST
API and, when `SAVE_CSV` is set, writes them to
`data/ig_<epic-slug>_<resolution-suffix>_<timestamp>.csv` (a new timestamped
file per run, named from `EPIC` — `_epic_slug()` in `igmarket/config.py`
...
## `05_download_yahoo_finance.ipynb` (standalone, unrelated to the DB pipeline)

Pulls OHLCV candles for a Yahoo Finance ticker (default `^NDX`, the NASDAQ
100 *index* — not the IG CFD `01`–`04` use) via `igmarket/yahoo_finance.py`'s
...
returns no rows. Deliberately independent of `igmarket.config` / `.env` —
...
`data/yahoo_<ticker-slug>_<interval>_<timestamp>.csv` (e.g. `^NDX` ->
`ndx`) via `save_csv()`. Yahoo has no bid/ask, so this CSV is a single
`Open`/`High`/`Low`/`Close`/`Volume` series — a different shape from the IG
CSVs (`igmarket/constants/csv_headers.py`) and **not** importable by
`02_import_csv_to_db.ipynb`. `igmarket/timeutil.py` (`parse_utc`,
